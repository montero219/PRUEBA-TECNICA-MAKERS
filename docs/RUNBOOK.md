...
```powershell
dotnet user-secrets set "ConnectionStrings:Atlas" "Host=localhost;Port=5433;Database=atlas_pars;Username=atlas_pars;Password=<nuevo-password>" --project src\Atlas.PARS.Api\Atlas.PARS.Api.csproj
```
...
```powershell
dotnet ef database update <MigracionAnterior> --project src\Atlas.PARS.Api\Atlas.PARS.Api.csproj
```
