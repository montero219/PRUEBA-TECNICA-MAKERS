...
La clave activa se inyecta por configuracion (`FirmaDecisiones:KeyId` y `FirmaDecisiones:ClaveActivaBase64`), nunca hardcodeada: user-secrets en desarrollo local, variable de entorno en el contenedor. El firmador falla cerrado al arrancar la aplicacion (no en el primer request) si la clave no esta configurada o no es Base64 valido, siguiendo el mismo principio de fallo cerrado del ADR 0003. En produccion, esa configuracion se reemplazaria por una referencia a Azure Key Vault (o el KMS equivalente) sin cambiar la interfaz `IFirmadorDecisionesAutorizacion`.
